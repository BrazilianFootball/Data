...
def extract_games(competition, cod, year, files):
    ...
    while True:
        ...
        try:
            ...
            pdf = get_pdf(
                f"https://conteudo.cbf.com.br/sumulas/{year}/{cod}{game}se.pdf"
            )
...
def scrape(competitions, min_year, max_year, files, max_time=60, cleaning=True):
    ...
    for competition in competitions:
        ...
        for year in range(min_year, max_year + 1):
            ...
            end_scrape = time()
            if end_scrape - start_scrape > max_time:
...
def extract(competitions, min_year, max_year, cleaning=True):
    ...
    cont_fail = 0
    for competition in competitions:
        if competition not in exceptions:
            exceptions[competition] = dict()
        competition = competition[0]
        for year in range(min_year, max_year + 1):
            if str(year) not in exceptions[competition]:
                exceptions[competition][str(year)] = dict()
            if cleaning:
                clear()
            year = str(year)
            ...
            games = {}
            count_end = 0
            if f"{competition}_{year}_games.json" in os.listdir("./processed/"):
                files = glob(f"./raw/{competition}/{year}/CSVs/*.csv")
                if len(files) == 0:
                    continue
                latest_file = max(files, key=os.path.getmtime)
                mod_time = os.path.getmtime(latest_file)
                if mod_time < os.path.getmtime(
                    f"./processed/{competition}_{year}_games.json"
                ):
                    continue
            ...
            while True:
                ...
                if str(game).zfill(3) in exceptions[competition][year]:
                    if exceptions[competition][year][str(game).zfill(3)] != {}:
                        games[str(game).zfill(3)] = exceptions[competition][year][
                            str(game).zfill(3)
                        ]
                        summary.append(
                            [
                ...
                try:
                    with open(
                        f"./raw/{competition}/{year}/CSVs/{str(game).zfill(3)}.csv", "r"
                    ) as f:
                    ...
                    if not f_club:
                        errors.append(
                            f"Error during extracting clubs from game {game} of {year}'s {competition}."
                        )
                    elif not f_result:
                        errors.append(
                            f"Error during extracting result from game {game} of {year}'s {competition}."
                        )
                    elif not f_players:
                        errors.append(
                            f"Error during extracting players from game {game} of {year}'s {competition}."
                        )
                    elif not f_goals:
                        errors.append(
                            f"Error during extracting goals from game {game} of {year}'s {competition}."
                        )
                    elif not f_changes:
                        errors.append(
                            f"Error during extracting changes from game {game} of {year}'s {competition}."
                        )
                    else:
                        errors.append(
                            f"Error in game {game} of {year}'s {competition}."
                        )

            with open(f"./processed/{competition}_{year}_games.json", "w") as f:
                json.dump(games, f)
            if len(summary) == 1:
                continue
...
def catch_squads(competitions, min_year, max_year, cleaning=True):
    ...
    for competition in competitions:
        ...
        for year in range(min_year, max_year + 1):
            ...
            if f"{competition}_{year}_squads.json" in os.listdir("processed/"):
                files = glob(f"./raw/{competition}/{year}/CSVs/*.csv")
                if len(files) == 0:
                    continue
                latest_file = max(files, key=os.path.getmtime)
                mod_time = os.path.getmtime(latest_file)
                if mod_time < os.path.getmtime(
                    f"./processed/{competition}_{year}_squads.json"
                ):
                    continue

            squads = {}
            with open(f"./processed/{competition}_{year}_games.json") as f:
                games = json.load(f)
            ...
            for game in games:
                ...
                try:
                    yellow_cards = treat_game_events(
                        games[game]["Yellow cards"], home, away
                    )
                except Exception as e:
                ...
                for player in players:
                    ...
                    if player[1] == home:
                        ...
                        game_club = "Away"
                    if len(squads[game][0][game_club]["Squad"]) == 11:
                        continue
                ...
                for i, change in enumerate(changes):
                    ...
                    try:
                        player_in = (
                            game_players[club][player_in] if player_in != "" else ""
                        )
                    except KeyError:
                        errors.append(
                            f"Can't find player {player_in} of {club} in {game} of {year}'s {competition}"
                        )
                    try:
                            ...
                            if minute > actual_minute and minute <= time:
                                if player == "":
                                    ...
                                    try:
                                        ...
                                        errors.append(
                                            f"Not found player {player} from {team} on game {game} of {year}'s {competition}"
                                        )
