...
def treat_time(time_string):
    ...
    time = 0
    if 'INT' in time_string: return 45
...
def catch_yellow_cards(text):
    ...
    text = text.replace('\'', '')
    return re.findall('\+*\d+[:\d+]*\s*\dT\s*\d+\s*[a-zA-ZÀ-ÿ\-\. 0-9]+\/[A-Z]{2}', text)
...
def catch_goals(text):
    ...
    goals  = re.findall('\+*\d+[:\d+]*\s*\dT\s*[\d+\s*]*NR[a-zA-ZÀ-ÿ\-\. 0-9]+\/[A-Z]{2}', text)
    goals += re.findall('\+*\d+[:\d+]*\s*\dT\s*[\d+\s*]*PN[a-zA-ZÀ-ÿ\-\. 0-9]+\/[A-Z]{2}', text)
    goals += re.findall('\+*\d+[:\d+]*\s*\dT\s*[\d+\s*]*CT[a-zA-ZÀ-ÿ\-\. 0-9]+\/[A-Z]{2}', text)
    goals += re.findall('\+*\d+[:\d+]*\s*\dT\s*[\d+\s*]*FT[a-zA-ZÀ-ÿ\-\. 0-9]+\/[A-Z]{2}', text)
...
def treat_event(event, home, away):
    ...
    if ' / ' not in event:
        ...
        elif '/ ' in event: event = event.replace('/ ', ' / ')
        elif '/' in event: event = event.replace('/', ' / ')
    
    event = treat_club(event)
    if '1T' not in event and '2T' not in event:
        if '3T' in event: event = event.replace('3T', '2T')
        elif '0T' in event: event = event.replace('0T', '1T')
    
    time_string = re.findall('\+*\d+:\d+\s*[A-Z0-9]+T|\+*\d+\s*\dT', event)[0]
    time = treat_time(time_string)
    player_number = re.findall('\d+:\d+\s*[A-Z0-9]+T\s*(\d+)|\+\d+\s*\dT\s*(\d+)', event)
    ...
    if home in event and away not in event: return time, player_number, home
    elif home not in event and away in event: return time, player_number, away
    
    if 'Flamengo / RJ' in [home, away] and 'Flamengo' in event: return time, player_number, 'Flamengo / RJ'
    if 'Vasco da Gama / RJ' in [home, away] and 'Vasco da Gama' in event: return time, player_number, 'Vasco da Gama / RJ'
    if 'Fluminense / RJ' in [home, away] and 'Fluminense' in event: return time, player_number, 'Fluminense / RJ'
    if 'Botafogo / RJ' in [home, away] and 'Botafogo' in event: return time, player_number, 'Botafogo / RJ'
    if 'Madureira / RJ' in [home, away] and 'Madureira' in event: return time, player_number, 'Madureira / RJ'
    if 'Resende / RJ' in [home, away] and 'Resende' in event: return time, player_number, 'Resende / RJ'
    if 'Nova Iguaçu / RJ' in [home, away] and 'Nova Iguaçu' in event: return time, player_number, 'Nova Iguaçu / RJ'
    if 'Macaé / RJ' in [home, away] and 'Macae' in event: return time, player_number, 'Macaé / RJ'
    if 'Duque de Caxias / RJ' in [home, away] and 'Duque de Caxias' in event: return time, player_number, 'Duque de Caxias / RJ'
    if 'Palmeiras / SP' in [home, away] and 'Palmeiras' in event: return time, player_number, 'Palmeiras / SP'
    if 'São Paulo / SP' in [home, away] and 'São Paulo' in event: return time, player_number, 'São Paulo / SP'
    if 'Corinthians / SP' in [home, away] and 'Corinthians' in event: return time, player_number, 'Corinthians / SP'
    if 'Santos / SP' in [home, away] and 'Santos' in event: return time, player_number, 'Santos / SP'
    if 'Red Bull Bragantino / SP' in [home, away] and 'Bragantino' in event: return time, player_number, 'Red Bull Bragantino / SP'
    if 'Sao Bernardo / SP' in [home, away] and 'Sao Bernardo' in event: return time, player_number, 'Sao Bernardo / SP'
    if 'São Bento / SP' in [home, away] and 'São Bento' in event: return time, player_number, 'São Bento / SP'
    if 'Ponte Preta / SP' in [home, away] and 'Ponte Preta' in event: return time, player_number, 'Ponte Preta / SP'
    if 'Portuguesa / SP' in [home, away] and 'Portuguesa' in event: return time, player_number, 'Portuguesa / SP'
    if 'Santo André / SP' in [home, away] and 'Santo André' in event: return time, player_number, 'Santo André / SP'
    if 'São Caetano / SP' in [home, away] and 'São Caetano' in event: return time, player_number, 'São Caetano / SP'
    if 'Penapolense / SP' in [home, away] and 'Penapolense' in event: return time, player_number, 'Penapolense / SP'
    if 'Botafogo / SP' in [home, away] and 'Botafogo' in event: return time, player_number, 'Botafogo / SP'
    if 'Grêmio Barueri / SP' in [home, away] and 'Grêmio Barueri' in event: return time, player_number, 'Grêmio Barueri / SP'
    if 'Mogi Mirim / SP' in [home, away] and 'Mogi Mirim' in event: return time, player_number, 'Mogi Mirim / SP'
    ...
    if 'Tupi / MG' in [home, away] and 'Tupi' in event: return time, player_number, 'Tupi / MG'
    if 'Araxá / MG' in [home, away] and 'Araxá' in event: return time, player_number, 'Araxá / MG'
    if 'Betim / MG' in [home, away] and 'Betim' in event: return time, player_number, 'Betim / MG'
    if 'Boa / MG' in [home, away] and 'Boa' in event: return time, player_number, 'Boa / MG'
    if 'Aracruz / ES' in [home, away] and 'Aracruz' in event: return time, player_number, 'Aracruz / ES'
    if 'Internacional / RS' in [home, away] and 'Internacional' in event: return time, player_number, 'Internacional / RS'
    if 'Grêmio / RS' in [home, away] and 'Grêmio' in event: return time, player_number, 'Grêmio / RS'
    if 'Juventude / RS' in [home, away] and 'Juventude' in event: return time, player_number, 'Juventude / RS'
    if 'Lajeadense / RS' in [home, away] and 'Lajeadense' in event: return time, player_number, 'Lajeadense / RS'
    if 'Caxias / RS' in [home, away] and 'Caxias' in event: return time, player_number, 'Caxias / RS'
    if 'Athletico Paranaense / PR' in [home, away] and 'Atlético' in event: return time, player_number, 'Athletico Paranaense / PR'
    if 'Coritiba / PR' in [home, away] and 'Coritiba' in event: return time, player_number, 'Coritiba / PR'
    if 'Paraná / PR' in [home, away] and 'Paraná' in event: return time, player_number, 'Paraná / PR'
    if 'Arapongas / PR' in [home, away] and 'Arapongas' in event: return time, player_number, 'Arapongas / PR'
    if 'Londrina / PR' in [home, away] and 'Londrina' in event: return time, player_number, 'Londrina / PR'
    if 'J. Malucelli / PR' in [home, away] and 'J. Malucelli' in event: return time, player_number, 'J. Malucelli / PR'
    if 'Chapecoense / SC' in [home, away] and 'Chapecoense' in event: return time, player_number, 'Chapecoense / SC'
    if 'Figueirense / SC' in [home, away] and 'Figueirense' in event: return time, player_number, 'Figueirense / SC'
    if 'Criciúma / SC' in [home, away] and 'Criciuma' in event: return time, player_number, 'Criciúma / SC'
    if 'Avaí / SC' in [home, away] and 'AVAÍ' in event: return time, player_number, 'Avaí / SC'
    if 'Metropolitano / SC' in [home, away] and 'Metropolitano' in event: return time, player_number, 'Metropolitano / SC'
    if 'Marcílio Dias / SC' in [home, away] and 'Marcílio Dias' in event: return time, player_number, 'Marcílio Dias / SC'
    if 'Avaí / SC' in [home, away] and 'Avaí' in event: return time, player_number, 'Avaí / SC'
    if 'Joinville / SC' in [home, away] and 'Joinville' in event: return time, player_number, 'Joinville / SC'
    if 'Fortaleza / CE' in [home, away] and 'Fortaleza' in event: return time, player_number, 'Fortaleza / CE'
    if 'Ceará / CE' in [home, away] and 'Ceará' in event: return time, player_number, 'Ceará / CE'
    if 'Icasa / CE' in [home, away] and 'Icasa' in event: return time, player_number, 'Icasa / CE'
    if 'Tiradentes / CE' in [home, away] and 'Tiradentes' in event: return time, player_number, 'Tiradentes / CE'
    if 'Guarany / CE' in [home, away] and 'Guarany' in event: return time, player_number, 'Guarany / CE'
    if 'Vitória da Conquista / BA' in [home, away] and 'Vitória da Conquista' in event: return time, player_number, 'Vitória da Conquista / BA'
    if 'Vitória / BA' in [home, away] and 'Vitória' in event: return time, player_number, 'Vitória / BA'
    if 'Bahia / BA' in [home, away] and 'Bahia' in event: return time, player_number, 'Bahia / BA'
    if 'Juazeirense / BA' in [home, away] and 'Juazeirense' in event: return time, player_number, 'Juazeirense / BA'
    if 'ABC / RN' in [home, away] and 'A.b.c.' in event: return time, player_number, 'ABC / RN'
    if 'América / RN' in [home, away] and 'América' in event: return time, player_number, 'América / RN'
    if 'Potiguar / RN' in [home, away] and 'Potiguar' in event: return time, player_number, 'Potiguar / RN'
    if 'Baraúnas / RN' in [home, away] and 'Baraúnas' in event: return time, player_number, 'Baraúnas / RN'
    if 'Salgueiro / PE' in [home, away] and 'Salgueiro' in event: return time, player_number, 'Salgueiro / PE'
    if 'Sport / PE' in [home, away] and 'Sport' in event: return time, player_number, 'Sport / PE'
    if 'Central / PE' in [home, away] and 'Central' in event: return time, player_number, 'Central / PE'
    if 'Santa Cruz / PE' in [home, away] and 'Santa Cruz' in event: return time, player_number, 'Santa Cruz / PE'
    if 'Ypiranga / PE' in [home, away] and 'Ypiranga' in event: return time, player_number, 'Ypiranga / PE'
    if 'Náutico / PE' in [home, away] and 'Náutico' in event: return time, player_number, 'Náutico / PE'
    if 'Confiança / SE' in [home, away] and 'Confiança' in event: return time, player_number, 'Confiança / SE'
    if 'Sergipe / SE' in [home, away] and 'Sergipe' in event: return time, player_number, 'Sergipe / SE'
    if 'ASA / AL' in [home, away] and 'A.s.a.' in event: return time, player_number, 'ASA / AL'
    if 'CSA / AL' in [home, away] and 'C.s.a.' in event: return time, player_number, 'CSA / AL'
    if 'CRB / AL' in [home, away] and 'C.r.b.' in event: return time, player_number, 'CRB / AL'
    if 'Botafogo / PB' in [home, away] and 'Botafogo' in event: return time, player_number, 'Botafogo / PB'
    if 'Treze / PB' in [home, away] and 'Treze' in event: return time, player_number, 'Treze / PB'
    if 'Parnahyba / PI' in [home, away] and 'Parnahyba' in event: return time, player_number, 'Parnahyba / PI'
    if 'Sampaio Corrêa / MA' in [home, away] and 'Sampaio Correa' in event: return time, player_number, 'Sampaio Corrêa / MA'
    if 'Maranhão / MA' in [home, away] and 'Maranhão' in event: return time, player_number, 'Maranhão / MA'
    if 'Cuiabá / MT' in [home, away] and 'Cuiabá' in event: return time, player_number, 'Cuiabá / MT'
    if 'Luverdense / MT' in [home, away] and 'Luverdense' in event: return time, player_number, 'Luverdense / MT'
    if 'Mixto / MT' in [home, away] and 'Mixto' in event: return time, player_number, 'Mixto / MT'
    if 'Naviraiense / MS' in [home, away] and 'Naviraiense' in event: return time, player_number, 'Naviraiense / MS'
    if 'Águia Negra / MS' in [home, away] and 'Aguia Negra' in event: return time, player_number, 'Águia Negra / MS'
    if 'Goiás / GO' in [home, away] and 'Goiás' in event: return time, player_number, 'Goiás / GO'
    if 'Aparecidense / GO' in [home, away] and 'Aparecidense' in event: return time, player_number, 'Aparecidense / GO'
    if 'CRAC / GO' in [home, away] and 'C.r.a.c.' in event: return time, player_number, 'CRAC / GO'
    if 'Goianesia / GO' in [home, away] and 'Goianesia' in event: return time, player_number, 'Goianesia / GO'
    if 'Vila Nova / GO' in [home, away] and 'Vila Nova' in event: return time, player_number, 'Vila Nova / GO'
    if 'Atlético / GO' in [home, away] and 'Atlético' in event: return time, player_number, 'Atlético / GO'
    if 'Brasília / DF' in [home, away] and 'Brasília' in event: return time, player_number, 'Brasília / DF'
    if 'Brasiliense / DF' in [home, away] and 'Brasiliense' in event: return time, player_number, 'Brasiliense / DF'
    if 'Nacional / AM' in [home, away] and 'Nacional' in event: return time, player_number, 'Nacional / AM'
    if 'Ypiranga / AP' in [home, away] and 'Ypiranga' in event: return time, player_number, 'Ypiranga / AP'
    if 'Paysandu / PA' in [home, away] and 'Paysandu' in event: return time, player_number, 'Paysandu / PA'
    if 'Águia de Marabá / PA' in [home, away] and 'Aguia' in event: return time, player_number, 'Águia de Marabá / PA'
    if 'Paragominas / PA' in [home, away] and 'Paragominas' in event: return time, player_number, 'Paragominas / PA'
    if 'Gurupi / TO' in [home, away] and 'Gurupi' in event: return time, player_number, 'Gurupi / TO'
    if 'Plácido de Castro / AC' in [home, away] and 'Plácido de Castro' in event: return time, player_number, 'Plácido de Castro / AC'
    if 'Rio Branco / AC' in [home, away] and 'Rio Branco' in event: return time, player_number, 'Rio Branco / AC'
    if 'Genus / RO' in [home, away] and 'Genus' in event: return time, player_number, 'Genus / RO'
    if 'Nautico / RR' in [home, away] and 'Nautico' in event: return time, player_number, 'Nautico / RR'
